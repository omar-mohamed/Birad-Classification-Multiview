...
def get_metrics_from_generator(generator,threshold=0.5, verbose=1):
    y_hat = multiview_model.predict(generator, steps=generator.steps, workers=FLAGS.generator_workers,
...
if FLAGS.multi_label_classification:
    multiview_model.compile(loss='binary_crossentropy',
                         metrics=[metrics.BinaryAccuracy(threshold=FLAGS.multilabel_threshold)])
else:
    multiview_model.compile(loss='sparse_categorical_crossentropy', metrics=['accuracy'])
